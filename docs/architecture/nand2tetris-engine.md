...
`NAND -> Boolean logic -> routing -> 16-bit logic -> arithmetic -> DFF/BIT/register -> memory -> instruction decoder -> CPU -> computer`
...
- `MEMORY` exposes the Hack data-memory boundary, while `SCREEN` and `KEYBOARD` are explicit device boundaries.
- `HACK CPU` owns the A register, D register, program counter, Hack ALU decoding, destination writes, and jump decision. Its visible contract is `IN M`, `INSTRUCTION`, `RESET`, `CLOCK` to `OUT M`, `WRITE M`, `ADDRESS M`, `PC`.
