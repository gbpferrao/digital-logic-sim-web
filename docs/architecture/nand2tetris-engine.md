...
`storage/nand2tetris-native/` is a generated companion track, produced by `scripts/build-nand2tetris-native-assets.mjs`. It starts from the same records, rewrites exact basic-gate wrapper instances to native catalog primitives, remaps their endpoint pin IDs, and runs the same leaf-to-root layout pass again. The original directory is intentionally preserved as the NAND-derived comparison track. The native-primitives tree is copied to `public/examples/nand2tetris-native/` and is useful when studying higher-level compositions without expanding every basic gate into a teaching wrapper.
