...
function inspectorAction(action, iconName, label, title = label) {
...
function renderLibrary() {
  ...
  root.innerHTML = "";
  for (const group of getCollectionGroups()) {
    ...
    const heading = document.createElement("div"); heading.className = "library-group-title"; heading.textContent = group.name; section.appendChild(heading);
    chips.forEach((name) => {
      const desc = getDescription(project, name); if (!desc) return;
      const button = document.createElement("button"); button.className = "chip-button"; button.dataset.chip = name;
      const swatch = document.createElement("span"); swatch.className = "chip-swatch"; swatch.style.background = desc.colour;
      const label = document.createElement("span"); label.className = "chip-name"; label.textContent = name;
      const meta = document.createElement("span"); meta.className = "chip-meta"; meta.textContent = `${desc.inputPins.length}/${desc.outputPins.length}`;
      button.append(swatch, label, meta); section.appendChild(button);
    });
    root.appendChild(section);
  }
  if (!root.children.length) root.innerHTML = `<div class="empty-inspector">No chips match “${search}”.</div>`;
...
function toggleCollectionPopup(name, anchor = null) {
  ...
  const collection = getCollectionGroups().find((item) => item.name === name);
...
function reorderCollectionTabs(sourceName, targetName) {
  ...
  const names = getCollectionGroups().map((group) => group.name);
