...
- Click a pin to start or finish a compatible wire. Drag from a pin directly to another pin to finish without a separate click. The Wire tool can also start on empty canvas, leaving a visible loose endpoint when the wire is not attached. Drag that loose endpoint to reposition it; while a wire is active, click empty canvas to add route points, or start from an existing wire to create a branch. Right-click or Escape cancels the unfinished interaction.
