...
- `src/simulation-timeline.js` contains the `SimulationBake` lifecycle: bounded recording sessions, semantic interaction history, stability detection, exact execution head, scrub branching, and frame factories. The filename is historical; the removed `SimulationTimeline` alias is not part of the runtime contract.
- `src/simulation-trace.js` summarizes per-tick signal/display changes and keeps a bounded engine/propagation trace alongside the visible checkpoint rail.
...
- `src/simulation-controller.js` coordinates Record/Stop, Clear, Step, scrub navigation, stability completion, preview invalidation, and the simulation timer without owning DOM layout.
...
- The editor uses the reference-style full-world view: a compact top command strip for MENU, project name/save state, tools, and Record/timeline controls; a lower bar with reorderable chip-group drop-ups (including CUSTOM); a secondary status bar for coordinates, zoom, FIT, X-RAY, and speed; and floating library/inspector drawers.
- RECORD is the visible label for the single official simulation action, called a bake in the runtime and documentation: it starts a recorded session, closes automatically when the visible state stabilizes, or becomes STOP while an indefinite circuit is still evolving. The session keeps semantic user actions, one bounded engine trace event per tick, propagation/change summaries, and meaningful visual checkpoints. CLEAR discards it. The previous/next visible-step buttons, exact step field, and scrubber operate only on the current bake. The Step control shows the exact engine tick, while the scrubber represents meaningful visual checkpoints. The project menu opens a centered Help dialog with the most important shortcuts and commands.
...
- Pan with middle mouse or Alt+left drag from any canvas surface, including over objects. Zoom with the wheel or Alt+right drag. `FIT` frames the current circuit, Ctrl/Command+R resets the camera, and Ctrl/Command+G toggles the grid.
- Click a pin to start or finish a compatible wire. Drag from a pin directly to another pin to finish without a separate click. A click-started wire follows the cursor until it reaches a target; press Enter to place it at the cursor as a loose endpoint, or press Escape to cancel. The Wire tool can also start on empty canvas, leaving visible loose endpoints when the wire is not attached. Drag either loose endpoint to reposition that endpoint node; while a wire is active, click empty canvas to add route points, or start from an existing wire to create a branch. Right-click or Escape cancels the unfinished interaction.
...
- `RECORD` starts a new recording; while it is active the same button becomes `STOP` and closes the current recording. Semantic input/key actions are carried into the next official bake, while every official engine tick records propagation counts, changed signals/displays, settle diagnostics, and a checkpoint when the visible state changes. Finite circuits stop automatically when their visible state stabilizes; `CLEAR` returns to evaluated step zero. The previous/next visible-step controls, `Step N`, and paused scrubber operate on that bake. The scrubber shows meaningful visual checkpoints while `Step N` remains the exact engine tick; it stays visible but disabled for a static or one-checkpoint bake. Set the steps-per-second rate with the numeric speed field in the secondary status bar; Space advances one step while paused; Ctrl/Command+Space starts recording or stops and keeps the current bake. Clicking an input device toggles its value, while the inspector exposes live pin state and device-specific settings. Hovering, selecting, panning, and other view-only gestures do not pollute the simulation record.
